...
export function RecipeBrowseStrip({
  ...
  const chipsRow = (
    <>
      <Link href={buildRecipesBrowseHref({ cuisine: recipe.cuisineType })} className={chipClass}>
      ...
      <Link href={buildRecipesBrowseHref({ difficulty: recipe.difficulty })} className={chipClass}>
        {formatDifficultyLabel(recipe.difficulty)}
      </Link>
      <Link href={buildRecipesBrowseHref({ maxTime: matchingTimeOption.key })} className={chipClass}>
  ...
  return (
    <>
      <section className="panel-light p-4 lg:hidden">
        ...
        <form method="get" action="/recipes" className="mt-3 flex gap-2">
          <input
            type="search"
            name="q"
            placeholder="Search by dish or ingredient"
            className={`${filterFieldClass} flex-1 placeholder:text-charcoal/45`}
          />
          <button className="inline-flex shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-flame to-ember px-4 py-3 text-sm font-semibold text-white">
            Search
          </button>
        </form>
        <div className="mt-3 flex flex-wrap gap-2">{chipsRow}</div>
      </section>

      <section className="panel-light hidden p-5 sm:p-6 lg:block">
        ...
        <div className="mt-4 flex flex-wrap gap-3">{chipsRow}</div>
      </section>
    </>
  );
}
