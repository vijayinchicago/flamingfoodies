  ...
  return (
    <section className="container-shell py-16">
          ...
          return (
            <div key={tier}>
              <div className="flex items-center gap-4">
                <h2 className="font-display text-2xl text-charcoal">{TIER_LABELS[tier]}</h2>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {tierBrands.map((brand) => (
                  <Link
                    ...
                    className="group rounded-[1.75rem] border border-charcoal/10 bg-charcoal/[0.03] p-6 transition hover:-translate-y-0.5 hover:border-charcoal/20 hover:bg-charcoal/[0.06]"
                  >
                    ...
                    <h3 className="mt-2 font-display text-3xl leading-tight text-charcoal">
                      {brand.name}
                    </h3>
                    <p className="mt-2 text-sm leading-6 text-charcoal/65">{brand.tagline}</p>
                    <p className="mt-3 text-xs font-semibold text-charcoal/40 group-hover:text-charcoal/65">
                      Full profile →
                    </p>
                  </Link>
                ))}
      ...
      <div className="mt-16 grid gap-4 rounded-[2rem] border border-charcoal/10 bg-charcoal/[0.03] p-8 sm:grid-cols-3">
        <div>
          <p className="font-display text-5xl text-charcoal">{brands.length}</p>
          <p className="mt-2 text-sm text-charcoal/60">Brands profiled</p>
        </div>
        <div>
          <p className="font-display text-5xl text-charcoal">
            {new Set(brands.map((b) => b.origin)).size}
          </p>
          <p className="mt-2 text-sm text-charcoal/60">Countries of origin</p>
        </div>
        <div>
          <p className="font-display text-5xl text-charcoal">
            {Math.min(...brands.map((b) => parseInt(b.founded) || 2024))}
          </p>
          <p className="mt-2 text-sm text-charcoal/60">Oldest brand founded</p>
        </div>
      </div>
      ...
      <div className="mt-12 flex flex-wrap gap-3">
        ...
        <Link href="/peppers" className="rounded-full border border-charcoal/15 px-5 py-3 text-sm font-semibold text-charcoal hover:border-charcoal/30 hover:text-white">
          Pepper encyclopedia
        </Link>
