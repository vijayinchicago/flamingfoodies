...
export default async function FestivalsPage() {
  ...
  return (
    <section className="container-shell py-16">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {featured.map((festival) => (
              <Link
                key={festival.slug}
                href={`/festivals/${festival.slug}`}
                className="group overflow-hidden rounded-[2rem] border border-charcoal/10 bg-charcoal/[0.04] p-6 transition hover:-translate-y-1 hover:border-charcoal/20"
              >
                <div className="flex items-start justify-between gap-3">
                  <p className={`text-xs uppercase tracking-[0.22em] ${REGION_COLORS[festival.region] ?? "text-ember"}`}>
                    {getRegionLabel(festival.region)}
            ...
            return (
              <div key={month}>
                <div className="flex items-center gap-4">
                  <h2 className="font-display text-2xl text-charcoal">{getMonthName(month)}</h2>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {festivals.map((festival) => (
                    <Link
                      key={festival.slug}
                      href={`/festivals/${festival.slug}`}
                      className="group rounded-[1.75rem] border border-charcoal/10 bg-charcoal/[0.03] p-5 transition hover:border-charcoal/20 hover:bg-charcoal/[0.06]"
                    >
                      ...
                      <p className="mt-1 text-xs text-charcoal/45">{festival.dateRange}</p>
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {festival.tags.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full border border-charcoal/8 px-2.5 py-0.5 text-[11px] text-charcoal/45"
                          >
                            {tag}
                          </span>
                        ))}
      ...
      {/* Stats strip */}
      <div className="mt-16 grid gap-4 rounded-[2rem] border border-charcoal/10 bg-charcoal/[0.03] p-8 sm:grid-cols-3">
        <div>
          <p className="font-display text-5xl text-charcoal">{festivals.length}</p>
          <p className="mt-2 text-sm text-charcoal/60">Festivals tracked</p>
        </div>
        <div>
          <p className="font-display text-5xl text-charcoal">
            {new Set(festivals.map((f) => f.stateCode)).size}
          </p>
          <p className="mt-2 text-sm text-charcoal/60">States represented</p>
        </div>
        <div>
          <p className="font-display text-5xl text-charcoal">
            {sortedMonths.length}
          </p>
          <p className="mt-2 text-sm text-charcoal/60">Months covered Jan – Dec</p>
        </div>
      </div>
      ...
      <div className="mt-12 flex flex-wrap gap-3">
        <Link
          href="/hot-sauces"
          className="rounded-full border border-charcoal/15 px-5 py-3 text-sm font-semibold text-charcoal hover:border-charcoal/30 hover:text-white"
        >
          Browse hot sauces
        </Link>
        <Link
