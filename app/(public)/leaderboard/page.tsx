...
export default async function LeaderboardPage() {
  ...
  return (
    <section className="container-shell py-16">
      ...
      />
      <div className="mt-10 overflow-hidden rounded-[2rem] border border-charcoal/10">
        <table className="min-w-full divide-y divide-charcoal/10">
          <thead className="bg-charcoal/5">
            <tr className="text-left text-xs uppercase tracking-[0.24em] text-charcoal/50">
          ...
          <tbody className="divide-y divide-charcoal/10 bg-charcoal/5">
            {profiles.map((profile, index) => (
              <tr key={profile.id}>
                <td className="px-6 py-5 text-sm text-charcoal/75">#{index + 1}</td>
                <td className="px-6 py-5">
                  <div className="font-semibold text-charcoal">{profile.displayName}</div>
                  <div className="text-sm text-charcoal/55">@{profile.username}</div>
                </td>
                <td className="px-6 py-5 text-sm capitalize text-charcoal/75">{profile.role}</td>
