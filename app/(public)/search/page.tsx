  ...
  return (
    <section className="container-shell py-16">
      ...
      <div className="max-w-4xl">
        ...
        <h1 className="mt-4 font-display text-4xl text-charcoal sm:text-5xl lg:text-6xl">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {quickLinks.map((item) => (
          <Link
            ...
            className="rounded-full border border-charcoal/12 bg-charcoal/[0.05] px-4 py-2 text-sm font-semibold text-charcoal transition hover:border-charcoal/25 hover:bg-charcoal/[0.1]"
          >
            {item.label}
          </Link>
        ))}
            ...
            <div className="mt-6 grid gap-4">
              {results.map((result) => (
                <Link
                  ...
                  href={result.href}
                  className="rounded-[1.75rem] border border-charcoal/10 bg-charcoal/[0.05] p-6 transition hover:-translate-y-1 hover:border-charcoal/20 hover:bg-charcoal/[0.08]"
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <p className="text-xs uppercase tracking-[0.22em] text-ember">
                      {formatType(result.type)}
                    </p>
                    <span className="text-sm text-charcoal/55">{result.meta}</span>
                  </div>
                  <h3 className="mt-3 font-display text-3xl text-charcoal sm:text-4xl">{result.title}</h3>
                  <p className="mt-3 max-w-3xl text-sm leading-7 text-charcoal/72">
                    {result.description}
                  </p>
                </Link>
              ))}
